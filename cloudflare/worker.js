...
async function authSchema(env){
  ...
  await env.DB.prepare("CREATE TABLE IF NOT EXISTS zx_users (id TEXT PRIMARY KEY,email TEXT NOT NULL UNIQUE,username TEXT NOT NULL UNIQUE,password_hash TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'active',role TEXT NOT NULL DEFAULT 'customer',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)").run();
  const columns = await env.DB.prepare("PRAGMA table_info(zx_users)").all();
  if (!columns.results.some(column => column.name === "role")) {
    await env.DB.prepare("ALTER TABLE zx_users ADD COLUMN role TEXT NOT NULL DEFAULT 'customer'").run();
  }
...
export default {
  async fetch(request, env) {
    try {
      ...
      if(url.pathname.startsWith("/api/admin/catalog") && !env.DB)
        return json({ok:false,error:"DB_BINDING_NOT_CONFIGURED"},503);
      if(url.pathname.startsWith("/api/auth/") || url.pathname.startsWith("/api/admin/catalog")) await authSchema(env);
